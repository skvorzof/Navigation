...
//  Navigation
//
//  Created by Dima Skvortsov on 12.08.2022.
//
    ...
    private func save(
        ...
        guard context.hasChanges else {
            if context.parent != nil {
                self.handler(
                    for: .failure(error: .error(description: "Context has not changes")),
                    using: context,
...
extension CoreDataCoordinator: DatabaseCoordinatable {
            ...
            keyedValues.enumerated().forEach { (index, keyedValues) in
                guard
                    let entityDescription = NSEntityDescription.entity(
                        forEntityName: String(describing: model.self),
                        in: self.saveContext)
                else {
                    self.mainContext.perform {
    ...
    func update<T>(_ model: T.Type, predicate: NSPredicate?, keyedValues: [String: Any], completion: @escaping (Result<[T], DatabaseError>) -> Void)
    where T: Storable {
